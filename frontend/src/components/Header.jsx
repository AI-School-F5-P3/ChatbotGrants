...
import { useAuthActions } from "../context/AuthContext"; // ✅ Importa el hook de autenticación
...
const ProfileMenu = () => {
  ...
  return (
    <Menu open={isMenuOpen} handler={setIsMenuOpen} placement="bottom-end">
      <MenuHandler>
        <Button variant="text" className="flex items-center gap-1 rounded-full capitalize  py-0.5 pr-0 pl-4 lg:ml-auto">
          Mª Rosa Cuenca
          <Avatar
            variant="circular"
            size="sm"
        ...
        {profileMenuItems.map(({ label, icon }) => (
          ...
          >
            {React.createElement(icon, {
              className: `h-4 w-4 ${label === "Salir" ? "text-red-500" : ""}`,
              strokeWidth: 2,
            })}
            <Typography
              as="span"
...
export const Header = () => {
  ...
  return (
    <header className="header w-screen bg-white sticky flex h-20 w-full items-center justify-between border-b">
      <Navbar color="transparent" className="max-w-full w-full rounded-none py-0 my-0 px-6 h-20">
        ...
        <Collapse open={openNav}>
          <div className="flex items-center gap-x-1">
            <Button fullWidth variant="gradient" size="sm" onClick={logout}> 
              <span>Salir</span> {/* ✅ Ahora "Salir" cierra sesión */}
            </Button>
          </div>
        </Collapse>
      </Navbar>
